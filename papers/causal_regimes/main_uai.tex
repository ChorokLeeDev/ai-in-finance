...
\maketitle
\begin{abstract}
We study regime-dependent predictive structure among equity factors using 35 years
of daily Fama-French data (1990--2024) and a Student-$t$ Hidden Markov Model
with 50 random starts. The primary regime fit is selected by a leakage-safe
likelihood-only criterion; a 2008-screened fit is reported only as sensitivity.

In the primary fit, HML $\to$ SMB is strongly significant in the Normal regime at
lag 1 ($p = 8.75 \times 10^{-9}$; HAC $p = 9.45 \times 10^{-8}$). Elevated-regime
evidence is directional but not confirmatory ($p = 0.004$ at lag 1), while Crisis
evidence is not significant ($p = 0.695$). Event validation shows 2/6 events with
statistical support and 4/6 with directional support. In the frozen 2013--2024
evaluation, aggregate results remain significant at $p = 0.041$ with 32.1\% Crisis
labels.

Exploratory FF25 checks show a positive spatial gradient ($\rho_s = 0.35$, permutation
$p = 0.046$), with Small\-/HighBM accounting for 39\% of HML $\to$ SMB lag-15
$\Delta R^2$.

The trading strategy is not profitable (Sharpe $=-0.07$).
A hybrid detector, combining the HMM with a realized-volatility override, improves
COVID detection (47.8\% vs.\ 0\% HMM-only).
It yields 5.60\% VaR violations and passes the Christoffersen conditional coverage
test with $p = 0.336$.
We interpret Granger findings as predictive precedence, not structural causality.
\end{abstract}
...
reverse channel (SMB$\to$HML, $z = 5.37$, $p < 10^{-6}$) in Normal, invisible
to standard Granger tests (Section~\ref{sec:robustness}).
...
\item \textbf{Mechanism Evidence:} FF 25 overlap shows the HML$\to$SMB link is
...
Hamilton~\cite{hamilton1989new} introduced Markov-switching models for business
cycles. Krolzig~\cite{krolzig1997markov} extends this to MS-VARs, which jointly
model regime-dependent dynamics. We use MS-VAR as a baseline single-stage analog
(Section~\ref{sec:msvar}).
...
with 50 EM initializations.
\emph{Primary selection (leakage-safe):} the fit with highest log-likelihood
across all 50 starts, regardless of event detection. This fit assigns 0\% of
2008, 2011, and 2020 stress windows to the Crisis regime (Table~\ref{tab:detection}).
...
Crisis: $p = 0.880$ at lag 5)---no 2008-based screening is involved in
pair selection for the primary analysis.
...
\begin{table}[t]
\centering
\caption[Regime statistics]{Regime Summary Statistics (1990--2024).\\
Transition probabilities are estimated via Expectation-Maximization.}
\label{tab:regimes}
\small
...
interpret Crisis-regime results as pertaining to this statistical state. The 2008-screened
sensitivity fit does align with calendar crises; we report it separately where noted.
...
Significance threshold: $p < 0.00033$ (Bonferroni-corrected for 30 factor pairs).}
\label{tab:main}
\small
\begin{tabular}{llccc}
\toprule
...
\bottomrule
\multicolumn{5}{l}{\footnotesize $^*$Below 0.01 but above Bonferroni threshold.} \\
\multicolumn{5}{l}{\footnotesize $\alpha/30 = 0.00033$.}
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Incremental $R^2$ from adding HML lags (1--15) to SMB autoregression\\
by regime.
...
\caption{All-pairs Granger causality by regime\\
($-\log_{10}(p)$, lag=5). Bordered cells with asterisks survive Bonferroni
correction. The strongest regime-differential pair is MKT$\to$SMB; HML$\to$SMB
appears as a secondary differential pattern.}
\label{fig:heatmap}
\end{figure}
...
The recomputed, leakage-safe primary fit yields a narrower conclusion: HML$\to$SMB is
strongly significant only in the Normal regime at lag 1 ($p = 8.75 \times 10^{-9}$;
HAC $p = 9.45 \times 10^{-8}$). Elevated evidence is suggestive but
non-confirmatory under the paper's multiple-testing threshold. Crisis evidence is null
at lag 1 ($p = 0.695$). The frozen out-of-sample aggregate test is positive at
conventional levels ($p = 0.041$), while event-level results remain mixed.
