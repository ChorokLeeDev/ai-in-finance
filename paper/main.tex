...
\subsection{Intervention Simulation (Proposed)}

A key advantage of FK grouping is enabling \textbf{intervention simulation}: predicting
how data quality improvements would reduce uncertainty. We outline the approach here;
full experimental validation is ongoing work.

\paragraph{Proposed Setup.}
For each FK group $g_i$:
\begin{enumerate}
    \item Compute predicted attribution $\alpha_i$ via noise injection
    \item Define reference set: samples with low uncertainty (bottom 20\%)
    \item Apply intervention: replace $g_i$ features with reference values
    \item Measure simulated uncertainty change $|\Delta u_i|$
    \item Compare predicted ranking vs.\ simulated ranking
\end{enumerate}

\paragraph{Why FK enables this (and correlation clustering doesn't).}
FK groups share a \textbf{common intervention point}---the data collection process for that table.
Improving DRIVER data quality affects \emph{all} DRIVER features simultaneously.
In contrast, correlation clusters group statistically similar features that may come from
different processes with no shared intervention mechanism.

\paragraph{Expected validation.}
If FK attribution is calibrated, we expect high correlation ($\rho > 0.7$) between
predicted attribution and simulated uncertainty reduction. This would validate
RelUQ's utility for \textbf{prioritizing data quality investments}.
