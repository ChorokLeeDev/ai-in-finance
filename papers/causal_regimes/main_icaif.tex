...
0.11 (binomial test), though this test has limited discriminative power at
$n = 6$.
...
\textbf{Result:} The frozen HMM assigns 32.1\% of the test period to Crisis.
Within the frozen Crisis label, HML$\to$SMB yields $p = 0.041$
($n = 794$ clean observations), meeting a conventional 5\% threshold.
...
\textbf{Interpretation.} The frozen-HMM aggregate result is statistically positive
at conventional levels, but event-level behavior is heterogeneous: COVID-2020
shows directional support ($p = 0.032$), while 2022 does not ($p = 0.472$).
We therefore treat frozen OOS evidence as supportive but not uniform across
all stress episodes.
...
\textbf{Consequence.} The hybrid detector activates on 14.9\% of test days with a 93.2\%
false-alarm rate---on most alert days no VaR violation occurs. Practitioners
must weigh correct tail coverage against unnecessary capital reserves on
non-violation alert days; we analyze the base-rate mechanics in the appendix
(Section~\ref{sec:risk_monitoring}, ``False alarms'' paragraph).
...
\begin{enumerate}
    ...
    (Section~\ref{sec:frozen_oos}) yields aggregate support at $p = 0.041$, but
    event-level support remains heterogeneous (e.g., COVID $p = 0.032$ vs.\ 2022
    $p = 0.472$).
