"""
H&M Domain Validation: FK Attribution Test
==========================================

Test FK Attribution on H&M Fashion Retail domain (8 FK groups).
Expected: EP domain (hierarchical product taxonomy) → should work
...
def run_validation(n_runs=5):
    """Run FK Attribution validation on H&M domain."""
    print("=" * 70)
    print("H&M DOMAIN VALIDATION")
    print("Testing FK Attribution on Fashion Retail")
    print("=" * 70)
    ...
    print(f"  FK groups: {len(fk_list)}")
    for fk in fk_list:
        print(f"    {fk}: {len(fk_grouping[fk])} cols")
    ...
    for run in range(n_runs):
        ...
        attr_sorted = sorted(attribution.items(), key=lambda x: -x[1])[:3]
        err_sorted = sorted(error_impact.items(), key=lambda x: -x[1])[:3]
        print(f"    Top Attribution: {[f'{k}:{v:.1f}%' for k,v in attr_sorted]}")
        print(f"    Top Error:       {[f'{k}:{v:.1f}%' for k,v in err_sorted]}")
        print(f"    Correlation: ρ = {corr:.3f}")

    # Summary
    print(f"\n{'='*70}")
    print("H&M VALIDATION SUMMARY")
    print("="*70)
    ...
    print(f"\n  Average Attribution per FK group (sorted):")
    avg_attrs = {}
    for fk in fk_list:
        avg_attrs[fk] = np.mean([a.get(fk, 0) for a in all_attributions])
    for fk, attr in sorted(avg_attrs.items(), key=lambda x: -x[1]):
        mean_err = np.mean([e.get(fk, 0) for e in all_errors])
        print(f"    {fk:15s}: Attr={attr:5.1f}%, Error={mean_err:5.1f}%")

    # Verdict
    if mean_corr >= 0.7:
        verdict = f"✓ STRONG: H&M is EP domain (ρ={mean_corr:.2f})"
    elif mean_corr >= 0.4:
        verdict = f"~ MODERATE: H&M shows EP behavior (ρ={mean_corr:.2f})"
    elif mean_corr >= 0:
        verdict = f"? WEAK: H&M shows weak EP behavior (ρ={mean_corr:.2f})"
    else:
        verdict = f"✗ NOT EP: H&M is not an Error Propagation domain (ρ={mean_corr:.2f})"
    ...
    output_path = RESULTS_DIR / 'hm_validation.json'
