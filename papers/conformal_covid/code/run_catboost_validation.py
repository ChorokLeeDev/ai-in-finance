"""
CatBoost Validation of SHAP Concentration Diagnostic
...
If CatBoost replicates (rho > 0.6), the diagnostic scope extends to 3 boosting
implementations: LGB (rho=0.833), XGB (rho=0.548), CatBoost.

Usage:
    PYTHONPATH=/Users/i767700/Github/ai-in-finance:$PYTHONPATH \
    /usr/bin/python3 -u papers/conformal_covid/code/run_catboost_validation.py

Output:
    papers/conformal_covid/results/catboost_validation.json
"""
...
OUTPUT_FILE = RESULTS_DIR / "catboost_validation.json"


def load_and_preprocess_data(task, sample_size=SAMPLE_SIZE, seed=SEED):
    """Load task data and preprocess features (same pipeline as RF/LGB/XGB)."""
...
def compute_catboost_shap_concentration(model, X_val, feature_names, max_samples=SHAP_SAMPLES):
    """Compute SHAP concentration for CatBoost model."""
    ...
    # CatBoost Pool for SHAP
    pool = Pool(X_sample, feature_names=feature_names)
    shap_values = model.get_feature_importance(pool, type='ShapValues')

    # CatBoost ShapValues returns either:
    # - 3D: (n_samples, n_classes, n_features+1) for multiclass
    # - 2D: (n_samples, n_features+1) for binary
    # The last column per class is the bias term
    n_features = len(feature_names)
    print(f"    SHAP shape: {shap_values.shape}, ndim={shap_values.ndim}, n_features={n_features}", flush=True)

    if shap_values.ndim == 3:
        # Already (n_samples, n_classes, n_features+1)
        sv = shap_values[:, :, :n_features]  # drop bias column
        mean_abs = np.abs(sv).mean(axis=(0, 1))  # average over samples and classes
    elif shap_values.shape[1] == n_features + 1:
        # Binary: (n_samples, n_features+1)
        sv = shap_values[:, :n_features]
        mean_abs = np.abs(sv).mean(axis=0)
    else:
        # 2D multiclass: (n_samples, (n_features+1) * n_classes) — flatten format
        actual_n_classes = shap_values.shape[1] // (n_features + 1)
        sv = shap_values.reshape(len(X_sample), actual_n_classes, n_features + 1)
        sv = sv[:, :, :n_features]
...
def run_single_task(task_name):
    """Run CatBoost validation for a single task."""
    ...
    # Train CatBoost
    print(f"  Training CatBoost (iterations=500)...", flush=True)
    t_cb = time.time()

    # Filter val to only classes present in training (subsampling may miss rare classes)
    train_classes = set(np.unique(y_data['train']))
    val_mask = np.array([y in train_classes for y in y_data['val']])
    val_X_filtered = X_data['val'][val_mask]
    val_y_filtered = y_data['val'][val_mask]
    n_filtered = (~val_mask).sum()
    if n_filtered > 0:
        print(f"  Filtered {n_filtered} val samples with unseen classes for early stopping", flush=True)
    ...
    train_pool = Pool(X_data['train'], label=y_data['train'], feature_names=feature_cols)
    val_pool = Pool(val_X_filtered, label=val_y_filtered, feature_names=feature_cols)

    model.fit(train_pool, eval_set=val_pool, verbose=False)
    cb_time = time.time() - t_cb
    print(f"  CatBoost trained in {cb_time:.1f}s (best iter: {model.get_best_iteration()})", flush=True)

    # Predict probabilities
    val_probs = model.predict_proba(X_data['val'])
    test_probs = model.predict_proba(X_data['test'])
    ...
    concentration, top_feature, shap_time = compute_catboost_shap_concentration(
    ...
    result = {
        'cb_concentration': round(concentration, 2),
        'cb_val_coverage': round(val_cov * 100, 2),
        'cb_test_coverage': round(test_cov * 100, 2),
        'cb_coverage_drop': round(coverage_drop * 100, 2),
        'cb_top_feature': top_feature,
        'num_classes': num_classes,
        'cb_train_time_s': round(cb_time, 1),
        'shap_time_s': round(shap_time, 1),
        'total_time_s': round(total_time, 1),
        'cb_best_iteration': model.get_best_iteration(),
...
def save_results(task_results, completed_tasks):
    ...
    if len(completed_tasks) >= 3:
        concs = [task_results[t]['cb_concentration'] for t in completed_tasks]
        drops = [task_results[t]['cb_coverage_drop'] for t in completed_tasks]
        rho, p = stats.spearmanr(concs, drops)
        output['cb_spearman_rho'] = round(float(rho), 3)
        output['cb_spearman_p'] = round(float(p), 4)
        print(f"\n  Current CatBoost Spearman rho={rho:.3f}, p={p:.4f} (n={len(completed_tasks)})", flush=True)
...
def main():
    print(f"\n{'='*70}", flush=True)
    print("CatBoost Validation of SHAP Concentration Diagnostic", flush=True)
    print(f"{'='*70}", flush=True)
    print(f"Tasks: {len(ALL_TASKS)}", flush=True)
    print(f"Comparing CatBoost vs LightGBM (existing: rho=0.833, p=0.010)", flush=True)
    print(f"{'='*70}\n", flush=True)
    ...
    # Final results with cross-model analysis
    print(f"\n{'='*70}", flush=True)
    ...
    if len(completed_tasks) >= 3:
        ...
        # Cross-model transfer: LGB concentration predicts CatBoost drops?
        transfer_rho, transfer_p = stats.spearmanr(lgb_concs, drops)

        print(f"\nSpearman correlation (n={len(completed_tasks)}):", flush=True)
        print(f"  CatBoost: rho={rho:.3f}, p={p:.4f}", flush=True)
        ...
        print(f"  LGB conc -> CatBoost drop: rho={transfer_rho:.3f}, p={transfer_p:.4f}", flush=True)

        final = {
            'tasks': task_results,
            'completed_tasks': completed_tasks,
            'n_tasks_completed': len(completed_tasks),
            'cb_spearman_rho': round(float(rho), 3),
            'cb_spearman_p': round(float(p), 4),
            ...
            'lgb_to_cb_transfer_rho': round(float(transfer_rho), 3),
            'lgb_to_cb_transfer_p': round(float(transfer_p), 4),
        }
        with open(OUTPUT_FILE, 'w') as f:
    ...
    # Table
    print(f"\n{'='*70}", flush=True)
    print(f"{'Task':<18} {'LGB Conc':>10} {'CB Conc':>10} {'LGB Drop':>10} {'CB Drop':>10}", flush=True)
    print(f"{'-'*70}", flush=True)
    for t in ALL_TASKS:
        if t in task_results and 'cb_concentration' in task_results[t]:
            r = task_results[t]
            print(f"{t:<18} {r['lgb_concentration']:>9.2f}% {r['cb_concentration']:>9.2f}% "
                  f"{r['lgb_coverage_drop']:>9.1f}% {r['cb_coverage_drop']:>9.1f}%", flush=True)
