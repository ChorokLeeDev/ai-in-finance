"""
50,000-permutation test — percentage-unit convention, primary seed 28.
Extended from permutation_10k_pctunit.py for tighter confidence intervals.
Shuffles regime labels while preserving factor time series structure.
"""
...
def main():
    print("=" * 60, flush=True)
    print("50,000-Permutation Test for Regime-Conditional Granger Causality", flush=True)
    print("=" * 60, flush=True)

    print("\nLoading FF data (percentage-unit — no /100 conversion)...", flush=True)
    df = download_ff_data()
    ...
    print(f"Fitting HMM seed {PRIMARY_SEED} on train data ({len(train_df)} days)...", flush=True)
    ...
    print(f"  Actual F={actual_F:.4f}, n_clean={n_clean}", flush=True)

    print(f"\nRunning {N_PERM:,} permutations (shuffling regime labels, preserving factor structure)...", flush=True)
    ...
    for i in range(N_PERM):
        ...
        if (i + 1) % 5000 == 0:
            ...
            current_se = np.sqrt(current_p * (1 - current_p) / (i + 1))
            print(f"  Permutation {i+1:,}/{N_PERM:,}: running p={current_p:.4f} (SE={current_se:.4f})", flush=True)
    ...
    ci_lower = max(0, perm_p - 1.96 * mc_se)
    ci_upper = min(1, perm_p + 1.96 * mc_se)

    result = {
        "description": "50,000-permutation OOS Elevated Granger test, percentage-unit convention, seed 28",
        ...
        "perm_95pct_F": round(perm_95, 4),
        "mc_se": round(mc_se, 5),
        "ci_95_lower": round(ci_lower, 4),
        "ci_95_upper": round(ci_upper, 4),
    }
    out_path = f"{RESULTS_DIR}/permutation_50k.json"
    ...
    print("=" * 60, flush=True)
    print(f"p = {perm_p:.4f}", flush=True)
    print(f"SE = {mc_se:.5f}", flush=True)
    print(f"95% CI = [{ci_lower:.4f}, {ci_upper:.4f}]", flush=True)
    print(f"n_valid_perm = {valid.sum():,}", flush=True)
