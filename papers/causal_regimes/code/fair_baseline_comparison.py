...
class StudentTHMM:
    ...
    def fit(self, X):
        ...
        for iteration in range(self.n_iter):
            ...
            self.A = np.sum(xi, axis=0) / np.sum(xi, axis=(0, 2), keepdims=True)
...
def load_ff_data_local():
    """Attempt to load from cache, otherwise download."""
...
def main():
    ...
    try:
        ...
        print(f"Download failed: {e}, attempting local load...")
    ...
    if df_regimes is None:
        ...
        hmm = StudentTHMM(n_regimes=3, n_iter=100, random_state=PRIMARY_SEED)
