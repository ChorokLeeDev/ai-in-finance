...
Permutation of $f_i$ partially disrupts $f_j$ (due to correlation), causing attribution
to ``leak'' between correlated features. Across random seeds, this leakage varies,
inflating $\text{Var}[\alpha_i]$ and $\text{Var}[\alpha_j]$.
Grouping eliminates this leakage by permuting $f_i, f_j$ together.
\end{proof}

\begin{theorem}[FK Grouping Stability]
\label{thm:stability}
Let $G = \{g_1, \ldots, g_m\}$ be FK groups with within-group correlation $\rho_g > \rho_0$
for some threshold $\rho_0 > 0$. Let $\alpha_g = \sum_{f \in g} \alpha_f$ be the group attribution.
Then:
\[
\sum_{g \in G} \text{Var}[\alpha_g] < \sum_{f} \text{Var}[\alpha_f]
\]
with the reduction proportional to within-group correlation.
\end{theorem}
...
correlated features, eliminating the instability.
