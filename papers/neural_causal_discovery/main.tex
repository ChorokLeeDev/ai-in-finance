...
\begin{abstract}
When do neural methods improve upon classical approaches for causal discovery in financial time series? We address this question through comprehensive experiments and propose \textbf{Adaptive Neural-Linear Granger (ANLG)}, a novel method that automatically selects between neural and linear approaches per edge based on detected nonlinearity. On synthetic nonlinear data, neural Granger significantly outperforms linear: smooth (+23.2\%, $p=0.015$) and threshold (+12.8\%, $p < 10^{-5}$). On real Fama-French data, classical methods perform comparably. ANLG achieves the best of both worlds: it matches neural performance on nonlinear data (+13.6\% vs linear, $p=0.008$) while providing interpretable per-edge method selection. Our results provide practical guidance: use ANLG for automatic method selection, or default to linear unless nonlinearity is confirmed.
\end{abstract}
...
We present a systematic study of when neural causal discovery methods outperform classical approaches, and propose \textbf{Adaptive Neural-Linear Granger (ANLG)}, a novel method for automatic method selection.

\textbf{Key findings:}
\begin{enumerate}
    \item \textbf{Synthetic nonlinear data}: Neural methods significantly outperform linear (threshold: +12.8\%, $p < 10^{-5}$; smooth: +23.2\%, $p=0.015$).

    \item \textbf{Real financial data}: Neither Fama-French factors nor circuit breaker events showed neural advantages, suggesting daily-frequency factor returns are adequately captured by linear methods.

    \item \textbf{ANLG contribution}: Our adaptive method matches neural performance on mixed data (+13.6\% vs linear, $p=0.008$) while providing interpretable per-edge method selection.
\end{enumerate}

\textbf{Practical guidance}: Use ANLG for automatic neural/linear selection, or default to simpler linear methods unless nonlinearity is empirically confirmed.

Our work provides the first systematic characterization of neural vs. linear trade-offs and a novel adaptive solution for computational finance applications.
