...
\begin{abstract}
When do neural methods improve upon classical approaches for causal discovery in financial time series? We address this question through comprehensive experiments comparing neural Granger causality against linear Granger and NOTEARS. On synthetic nonlinear data, neural Granger achieves F1=0.848, significantly outperforming both linear Granger (F1=0.464, $p<0.0001$) and NOTEARS (F1=0.286, $p<0.0001$). \textbf{On real cryptocurrency data, neural methods achieve +201\% improvement in market-structure alignment ($p=0.005$) and +7.2\% in prediction ($p=0.0001$).} On traditional Fama-French factors, linear methods suffice. We also propose Adaptive Neural-Linear Granger (ANLG) for automatic per-edge method selection. Our results provide clear guidance: use neural methods for markets with inherent nonlinearities (crypto, emerging assets), linear methods for efficient traditional markets.
\end{abstract}
...
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{F1} & \textbf{vs Neural} \\
\midrule
NOTEARS \cite{zheng2018dags} & 0.286 $\pm$ 0.00 & -66\% \\
Linear Granger & 0.464 $\pm$ 0.03 & -45\% \\
\textbf{Neural Granger} & \textbf{0.848} $\pm$ 0.03 & -- \\
\bottomrule
\multicolumn{3}{l}{\small Neural vs Linear: $p<0.0001$; Neural vs NOTEARS: $p<0.0001$}
\end{tabular}
\label{tab:nonlinear}
\end{table}

\paragraph{Finding.} Neural Granger significantly outperforms both linear Granger and NOTEARS on smooth nonlinear data ($p<0.0001$ for both comparisons). NOTEARS, designed for static DAG learning, performs poorly on time series data.
...
We present a systematic study comparing neural Granger causality against linear Granger and NOTEARS for causal discovery in financial time series.

\textbf{Key findings:}
\begin{enumerate}
    \item \textbf{Synthetic nonlinear data}: Neural Granger (F1=0.848) significantly outperforms both linear Granger (F1=0.464, $p<0.0001$) and NOTEARS (F1=0.286, $p<0.0001$).

    \item \textbf{Cryptocurrency data}: Neural methods achieve \textbf{+201\% improvement} in market-structure alignment ($p=0.005$) and \textbf{+7.2\% in prediction} ($p=0.0001$).

    \item \textbf{Traditional factors}: Fama-French equity factors are well-captured by linear methods.

    \item \textbf{ANLG}: Our adaptive method uses neural methods sparingly (20\% of edges at optimal $\tau$) while maintaining performance.
\end{enumerate}
...
Our work provides empirical validation of neural causal discovery on both synthetic and real financial data, along with actionable guidance for practitioners.
