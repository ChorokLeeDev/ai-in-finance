...
\begin{enumerate}
    \item \textbf{Pre-deployment diagnostic}: SHAP concentration correlates with conformal failure severity across 16 multiclass tasks in 9 domains: $\rho = 0.853$, $p < 0.001$ (Kendall $\tau = 0.667$; bootstrap 95\% CI $[0.50, 0.96]$). Within supply chain: $\rho = 0.833$, $p = 0.010$ ($n=8$). Outperforms native feature importance ($\rho = 0.667$) and ensemble disagreement ($\rho = 0.452$), though differences are not formally testable at $n=8$. Binary APS has a structural ceiling; concentration is diagnostic for multiclass settings.

    \item \textbf{Formal theory}: Theorem~\ref{thm:score_inflation} establishes APS score and coverage bounds worsen monotonically with $C$ under an additive decomposition model (Section~\ref{sec:theory}), verified on all 5 applicable tasks. Catastrophic tasks show \textit{decreasing} prediction entropy (confident misclassification), opposite to standard intuition.

    \item \textbf{Quantification}: Coverage drops 0.1\%--77.1\% across 8 tasks (all $p \leq 0.005$, 50 seeds), stable under LOO ($\rho \in [0.75, 0.96]$).

    \item \textbf{Operational framework (exploratory)}: Retraining partially recovers coverage (+19~pp, $p=0.036$, single seed; Holm-corrected $p=0.11$); the framework shows precision--recall trade-off across 30--45\% thresholds (Appendix~\ref{app:threshold}).

    \item \textbf{External validation}: The 40\% threshold transfers to 9 non-supply-chain datasets without tuning: 7/9 deterministic outcomes, Covertype correctly flagged catastrophic ($C=49.8\%$, 10/10 seeds), KDDCup99 intermediate-regime false negative.
...
External validation on 9 non-supply-chain datasets~\citep{dua2017uci} with the same pipeline (Appendix~\ref{app:external_data}) replicates the pattern: $\rho=0.853$, $p<0.001$ (Kendall $\tau=0.667$) across $n=16$ multiclass tasks. Covertype ($C=49.8\%$, 81.8~pp drop, 10/10 seeds) is the key catastrophic external case; 6 low-concentration domains are deterministic-robust; KDDCup99 ($C=21.1\%$, drop $=15.9\pm21.4$~pp) is the principal intermediate-regime false negative.
...
\begin{enumerate}
    \item \textbf{Compute} $C = \phi_1 / \sum_j \phi_j$ (Eq.~\ref{eq:concentration}). If $C \leq 40\%$: lower observed risk. If $C > 40\%$: check protective factors---secondary features with train-validation Jaccard $> 0.5$ and importance $> 15\%$; absent these, flag as \textbf{vulnerable}. If $C \in [35\%, 45\%]$: treat as \textbf{uncertain risk} with monitoring regardless of other factors.
...
\section{Conclusion}\label{sec:conclusion}\label{sec:discussion}
