...
\begin{enumerate}
    \item \textbf{Pre-deployment diagnostic}: SHAP concentration---the fraction of importance in the top feature---correlates with conformal failure severity: $\rho = 0.853$, $p < 0.001$ across 16 multiclass tasks in 9 domains (Kendall $\tau = 0.667$; bootstrap 95\% CI $[0.50, 0.96]$); $\rho = 0.833$, $p = 0.010$ within supply chain ($n=8$, stable under LOO: $\rho \in [0.75, 0.96]$). The 40\% threshold transfers to 9 non-supply-chain datasets without tuning (7/9 deterministic, Covertype correctly flagged at $C=49.8\%$). A model-sensitivity analysis ($\rho=0.833$ LightGBM vs.\ $\rho=0.30$ RF, $\rho=0.43$ MLP) identifies the scope: the diagnostic is specific to gradient-boosted classifiers where TreeSHAP exactly captures model structure (Section~\ref{sec:why_shap}).

    \item \textbf{Formal theory and comparative evidence}: Theorem~\ref{thm:score_inflation} establishes that APS score and coverage bounds worsen monotonically with $C$ (Section~\ref{sec:theory}), verified on all 5 applicable tasks; catastrophic tasks show \textit{decreasing} prediction entropy (confident misclassification). Standard shift indicators (MMD,\footnote{Population Stability Index (PSI): a monitoring metric standard in credit scoring practice~\citep{siddiqi2006credit}. PSI $>0.002$ indicates detectable nonzero shift; conventional significance threshold is $>0.1$.} C2ST, PSI) detect shift for all 8 tasks uniformly yet correlate $\rho \leq 0.19$ with coverage drop---shift detection does not predict failure severity.
...
\textbf{Why gradient-boosted models?} This is not a scope limitation but a structural finding. TreeSHAP~\citep{lundberg2020local} computes exact Shapley values via recursive path enumeration, making SHAP concentration a faithful measure of the model's actual conditional dependence on a single feature. Random forests average over many independent trees, each potentially with different top features, diluting concentration even when individual trees concentrate heavily on one feature---confirmed empirically by RF $\rho=0.30$ versus LightGBM $\rho=0.833$. MLP-SHAP (kernel or deep variants) approximates rather than exactly decomposes contributions, adding noise that obscures structural concentration. Gradient boosting's sequential additive training further amplifies single-feature dependence in a way parallel averaging (RF) does not. The model-sensitivity analysis ($\rho=0.833$ LightGBM, $0.667$ CatBoost, $0.548$ XGBoost vs.\ $0.30$ RF, $0.43$ MLP) thus identifies a boundary: \emph{SHAP concentration is a valid diagnostic precisely where TreeSHAP exactly captures model structure}---a property guaranteed for gradient-boosted trees and absent for RF and MLP architectures.
...
\begin{acknowledgements}
    ...
    commercial, or not-for-profit sectors. Code for all experiments and figure generation is publicly available at \url{https://github.com/ChorokLeeDev/conformal-covid}.
...
All experimental code---including SHAP concentration computation, conformal prediction evaluation, statistical tests, and figure generation---is publicly available at \url{https://github.com/ChorokLeeDev/conformal-covid}.
