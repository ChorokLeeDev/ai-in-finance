...
\begin{abstract}
When do neural methods improve upon classical approaches for causal discovery in financial time series? We address this practical question through comprehensive experiments. On synthetic nonlinear data, neural Granger significantly outperforms linear Granger: smooth nonlinearities (+23.2\%, $p=0.015$) and threshold nonlinearities (+12.8\%, $p < 10^{-5}$). On real Fama-French factor data and circuit breaker events, classical methods perform comparably or better. Our results provide clear practical guidance: \textbf{neural methods are valuable when strong nonlinearities are present}, but practitioners should verify this assumption before incurring additional complexity. We provide the first systematic characterization of when neural causal discovery outperforms classical approaches.
\end{abstract}
...
We present a systematic study of when neural causal discovery methods outperform classical approaches. Our key findings are:

\begin{enumerate}
    \item \textbf{Synthetic nonlinear data}: Neural methods significantly outperform linear baselines (threshold: +12.8\%, $p < 10^{-5}$; smooth: +23.2\%, $p=0.015$).

    \item \textbf{Real financial data}: Neither Fama-French factors nor circuit breaker events showed neural advantages, suggesting real financial relationships may be adequately captured by linear methods at daily frequency.

    \item \textbf{Practical guidance}: Neural methods are most valuable when strong, documented nonlinearities exist. Practitioners should default to simpler linear methods unless nonlinearity is confirmed.
\end{enumerate}

Our work provides the first systematic characterization of neural vs. linear causal discovery trade-offs, offering actionable guidance for computational finance applications.
