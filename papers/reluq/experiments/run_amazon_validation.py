"""
Run FK Uncertainty Framework validation on rel-amazon dataset

rel-amazon is an e-commerce platform with:
- users: User profiles
...
FK Classification:
- USERS: User features (correlational)
- ITEMS: Product features (causal - item characteristics drive churn/ltv)
- REVIEWS: Review text features (correlational)
"""
...
AMAZON_FK_CLASSIFICATION = {
    'USERS': 'correlational',
    'ITEMS': 'causal',
    'REVIEWS': 'correlational',
    'USER': 'correlational',
    'ITEM': 'causal',
...
def extract_features_generic(dataset, task, sample_size=2000):
...
def train_ensemble(X, y, n_models=5, n_classes=0, is_classification=True, seed=42):
    ...
    for i in range(n_models):
        ...
        if is_classification:
            unique = np.unique(y)
            boot = np.unique(y[idx])
            missing = set(unique) - set(boot)
            if missing:
...
def run_experiment(task_name, seed=42, sample_size=2000):
    print(f"\n{'='*60}")
    print(f"Running: rel-amazon / {task_name}")
    print(f"{'='*60}")

    dataset = get_dataset('rel-amazon', download=False)
    task = get_task('rel-amazon', task_name, download=False)
    ...
    for fk, c in sorted(fk_contribs.items(), key=lambda x: -x[1]['mean']):
        fk_type = AMAZON_FK_CLASSIFICATION.get(fk, 'unknown')
        print(f"    {fk}: {c['mean']:+.2f}% ± {c['std']:.2f}% ({fk_type})")

    causal = [c['mean'] for fk, c in fk_contribs.items() if AMAZON_FK_CLASSIFICATION.get(fk) == 'causal']
    corr = [c['mean'] for fk, c in fk_contribs.items() if AMAZON_FK_CLASSIFICATION.get(fk) == 'correlational']
...
def run_multi_seed(task_name, seeds=[42, 43, 44, 45, 46]):
    print(f"\n{'#'*60}")
    print(f"MULTI-SEED: rel-amazon / {task_name}")
    print(f"{'#'*60}")
...
def main():
    ...
    with open('test_results/amazon_validation_results.json', 'w') as f:
    ...
    print(f"\nResults saved to test_results/amazon_validation_results.json")
