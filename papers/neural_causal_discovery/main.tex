...
\paragraph{Key Finding.} On cryptocurrency data, neural methods \textbf{significantly outperform} linear methods for both causal discovery ($p=0.005$) and prediction ($p=0.0001$). This validates that neural causal discovery is valuable for \textbf{real financial markets with inherent nonlinearities}. The detected edges align with market microstructure: BTC movements propagate to altcoins through correlated liquidations, momentum traders, and exchange-specific dynamics. Traditional equity factors (Fama-French) are well-described by linear models due to market efficiency, but emerging asset classes like crypto exhibit nonlinear dynamics from leverage cascades and regime-dependent correlations.
...
Our cryptocurrency ground truth is based on market-structure priors (BTC leads alts, majors lead smaller coins) rather than causal identification. Neural methods require more computation and hyperparameter tuning. ANLG's interpretability advantage is not quantitatively evaluated.
