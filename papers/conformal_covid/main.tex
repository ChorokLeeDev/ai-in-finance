...
(SHAP) analysis, we find catastrophic failures correlate with \emph{single-feature
dependence} (rho = 0.714, p = 0.047). Catastrophic tasks concentrate importance
in one feature (4.5x increase), while robust tasks redistribute across many
(10-20x). Quarterly retraining restores catastrophic task coverage from 22\%
...
\keywords{Conformal Prediction, Distribution Shift, COVID-19, Uncertainty Quantification, Supply Chain, SHAP, Feature Importance, Model Retraining}
...
is misleading; median and Interquartile Range (IQR) provide robust statistics.}
...
\begin{itemize}
    \item \textbf{Hardware}: Standard Central Processing Unit (no Graphics Processing Unit required for LightGBM)
    ...
    \item \textbf{Peak memory}: $<$8 Gigabytes per task
\end{itemize}
